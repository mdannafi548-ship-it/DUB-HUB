  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, setter: (val: string) => void) => {
    ...
    uploadTask.on(
      ...
      (error) => {
        ...
        alert("Firebase Storage Permission Denied (Upload Failed)!\n\nBhai, apni shomvobot 'Firestore Database' e giye Storage er rules diyechen. Oita bhul!\n\nSothik niyom:\n1. Firebase console er BAM pasher menu theke 'Storage' e click korun (Firestore er niche othoba Build menu te).\n2. Storage er bhitor 'Rules' tab e jan.\n3. Sekhane lekha gulo kete eta din:\nrules_version = '2';\nservice firebase.storage {\n  match /b/{bucket}/o {\n    match /{allPaths=**} {\n      allow read, write: if true;\n    }\n  }\n}\n\n[Note: Firestore Database er rules aage ja chilo tai rekhe din, oita change korle app khulbei nah!]");
        setter("");
      },
      () => {
          ...
          <div className="bg-[#161b22] p-[30px] rounded-[20px] shadow-2xl w-full max-w-[450px] border border-[#8a2be2]/50 max-h-[85vh] overflow-y-auto relative animate-in fade-in zoom-in duration-200">
            ...
            {modal === 'join' && (
              <>
                ...
                <button onClick={submitApp} className="w-full py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-[#8a2be2] to-[#00ced1] hover:opacity-90">
                  Submit Application
                </button>
              </>
            )}
              ...
              <div className="bg-[#161b22] p-6 rounded-2xl border border-[#00ced1] md:col-span-2 shadow-[0_0_15px_rgba(0,206,209,0.1)]">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="bg-[#0b0e14] p-5 rounded-xl border border-[#333]">
                    ...
                    <div className="mb-3">
                      <label className="text-xs text-gray-400 mb-1 block">Choose file or Give thumbnail URL</label>
                      ...
                      <input type="text" placeholder="Or paste Thumbnail URL" value={clipForm.thumbSrc} onChange={e=>setClipForm(prev=>({...prev, thumbSrc: e.target.value}))} className="w-full p-2 bg-[#0b0e14] border border-[#333] rounded text-white text-sm" />
                      {clipForm.thumbSrc.startsWith('Uploading...') && <p className="text-[#00ced1] text-xs font-bold mt-1">{clipForm.thumbSrc}</p>}
                    </div>
                    <div className="mb-4">
                      <label className="text-xs text-gray-400 mb-1 block">Choose file or Give video URL</label>
                      ...
                      <input type="text" placeholder="Or paste Video URL" value={clipForm.videoSrc} onChange={e=>setClipForm(prev=>({...prev, videoSrc: e.target.value}))} className="w-full p-2 bg-[#0b0e14] border border-[#333] rounded text-white text-sm" />
                      {clipForm.videoSrc.startsWith('Uploading...') && <p className="text-[#8a2be2] text-xs font-bold mt-1">{clipForm.videoSrc}</p>}
                    </div>
                  ...
                  <div className="bg-[#0b0e14] p-5 rounded-xl border border-[#333]">
                    ...
                    <div className="mb-3">
                      ...
                      <input type="text" placeholder="Or paste Thumbnail URL" value={epiForm.thumbSrc} onChange={e=>setEpiForm(prev=>({...prev, thumbSrc: e.target.value}))} className="w-full p-2 bg-[#0b0e14] border border-[#333] rounded text-white text-sm" />
                      {epiForm.thumbSrc.startsWith('Uploading...') && <p className="text-[#ff4d6d] text-xs font-bold mt-1">{epiForm.thumbSrc}</p>}
                    </div>
                    <div className="mb-4">
                      <label className="text-xs text-gray-400 mb-1 block">Choose file or Give video URL</label>
                      ...
                      <input type="text" placeholder="Or paste Video URL" value={epiForm.videoSrc} onChange={e=>setEpiForm(prev=>({...prev, videoSrc: e.target.value}))} className="w-full p-2 bg-[#0b0e14] border border-[#333] rounded text-white text-sm" />
                      {epiForm.videoSrc.startsWith('Uploading...') && <p className="text-[#8a2be2] text-xs font-bold mt-1">{epiForm.videoSrc}</p>}
                    </div>
                     ...
                     <tbody>
                        {members.filter(m => m.name.toLowerCase().includes(teamSearch.toLowerCase()) || (m.email && m.email.toLowerCase().includes(teamSearch.toLowerCase()))).map(m => (
                          <tr key={m.id} className="border-b border-[#333] hover:bg-[#0b0e14]/50 transition-colors">
                            ...
                            <td className="p-3">
                              <select value={m.color || 'default'} onChange={(e) => setMembers(members.map(x => x.id === m.id ? {...x, color: e.target.value} : x))} className="bg-[#0b0e14] border border-[#555] rounded px-2 py-1 text-sm text-white">
                                <option value="default">Default</option>
                                <option value="green">Green</option>
                                <option value="blue">Blue</option>
                                <option value="dark-blue">Dark Blue</option>
                                <option value="golden-silver">Golden Silver</option>
                                {(m.rank === 'Admin' || m.rank === 'Moderator' || m.rank === 'Founder') && (
                                  <>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <select value={webConfig.adminColor} onChange={e=>setWebConfig({...webConfig, adminColor:e.target.value})} className="w-full p-2 mb-4 bg-[#0b0e14] border border-[#333] rounded text-white outline-none">
                        ...
                        <option value="admin-diamond">Diamond</option>
                        <option value="admin-reflect">Reflect</option>
                      </select>
                    </div>
